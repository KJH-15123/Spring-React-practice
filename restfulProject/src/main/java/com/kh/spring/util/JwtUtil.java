...
public class JwtUtil {
	...
	@Value("${jwt.secret:mySecretkeybackupTokenkey123asd21d1ddasdadjl2da}")
	...
	private SecretKey getSignKey() {
		...
		//같은 키로 서명한 토큰만 유효하다고 인정하기 위해(위조 방지)
	...
	public String generateToken(String userId) {
		...
		//JWT 토큰 빌더를 이용해서 생성
		return Jwts.builder()
				   .setSubject(userId) //subject : 토큰의 주체(누구에 대한 토큰인지, 일반적으로 사용자 ID나 이메일 사용)
				   .setIssuedAt(now) //issuedat : 토큰이 언제 발급되었는지 기록
				   .setExpiration(expiryDate) //expiration : 토큰이 언제 만료되는지 
				   .signWith(getSignKey()) //signWith : 생성된 암호화키로 토큰에 디지털 서명(위조 방지)
				   .compact();//최종적으로 JWT 문자열형태로 압축하여 반환한다. header.payload.signature 형태의 문자열 생성
	}
	...
	public String getUserIdFromToken(String token) {
		...
		Claims claims = Jwts.parserBuilder() //JWT파서 빌더 시작
						    .setSigningKey(getSignKey()) //서명검증을 위한 키 설정(토큰생성시 사용한 키를 이용해야함) 
						    .build() //파서 객체 생성 완료 
						    .parseClaimsJws(token)//위에서 생성한 파서객체 파싱하고 서명 검증하기 
	...
	public boolean validateToken(String token) {
		try {
			...
			Jwts.parserBuilder()
				...
				.build() //생성 
				.parseClaimsJws(token); //토큰 파싱및 검증 수행 
			//검증시 서명이 올바른지,토큰형식이 올바른지,토큰이만료되지 않았는지 확인한다.
			
			return true;//위에서 예외가 발생하지 않으면 유효 토큰
